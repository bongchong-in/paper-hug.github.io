document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Social Volunteer Modal ---
    const initSocialVolunteerModal = () => {
        const modal = document.getElementById('social-volunteer-modal');
        const closeModalBtn = document.getElementById('close-modal-btn');
        const socialTriggers = document.querySelectorAll('.social-media-volunteer-trigger');
        const modalContent = modal ? modal.querySelector('.modal-content') : null;
        const ignoreLink = document.getElementById('ignore-social-link');

        if (!modal || !closeModalBtn || socialTriggers.length === 0 || !modalContent || !ignoreLink) {
            return;
        }
        ...
        socialTriggers.forEach(trigger => {
            trigger.addEventListener('click', (e) => {
                ...
                const socialHref = trigger.dataset.href;

                if (socialName && socialHref) {
                    ignoreLink.textContent = `Ignore and take us to the social(${socialName}) handle`;
                    ignoreLink.href = socialHref;
                } else {
                    // Fallback in case attributes are missing, hide the link
